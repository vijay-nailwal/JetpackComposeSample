...
import com.example.jetpackcompose.data.CapitalInfo
import com.example.jetpackcompose.data.Car
import com.example.jetpackcompose.data.Currency
import com.example.jetpackcompose.data.Flags
import com.example.jetpackcompose.data.Idd
import com.example.jetpackcompose.data.Maps
import com.example.jetpackcompose.data.Name
import com.example.jetpackcompose.data.NativeName
import kotlinx.serialization.json.Json
import com.example.jetpackcompose.data.Languages as Languages1
...
class Converters {
    ...
    @TypeConverter
    fun fromLanguagesToJson(languages: Languages1?): String? {
        return languages?.let { jsonFormat.encodeToString(it) }
    }

    @TypeConverter
    fun fromJsonToLanguages(json: String?): Languages1? {
        return json?.let { jsonFormat.decodeFromString(it) }
    }
